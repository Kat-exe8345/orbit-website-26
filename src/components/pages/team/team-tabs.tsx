    ...
    return (
        <>
            ...
            <div className="lg:hidden relative flex flex-1 items-center w-full h-full">
                <button 
                    className={`flex flex-1 shrink-0 justify-center items-center w-full h-full bg-white text-black uppercase ${geistMono.className} text-sm tracking-wider font-medium transition-all duration-300 ease-in-out p-4`} 
                    onClick={() => setIsOpen(prev => !prev)}
                >
                    {activeTab === "all_teams" ? "All Teams" : Teams.find((team) => team.id === activeTab)?.label}
                ...
                <div className={`absolute flex flex-col top-full left-0 min-w-max w-full bg-white z-10 transition-all duration-300 ease-in-out border-y border-black ${isOpen ? 'block' : 'hidden'}`}>
                    <button
                        key="all_teams"
                        className={`flex shrink-0 justify-center items-center min-w-max w-full h-full text-white bg-[#040404] border-x border-b border-[#0f0f0f] uppercase ${geistMono.className} text-sm tracking-wider font-medium p-4`}
                        onClick={() => {
