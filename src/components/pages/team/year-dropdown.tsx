...
export default function YearDropdown({ Years, activeYear, setActiveYear }: { Years: { id: number; label: string }[], activeYear: number | null, setActiveYear: (year: number) => void }) {
    ...
    return (
        <div className={`relative flex flex-1 justify-around items-center max-w-max h-full border-x border-x-[#0f0f0f] border-b border-b-[#0f0f0f] transition-all duration-300 ease-in-out`}>
                    <span className={`${geistMono.className} text-sm font-medium text-black border-x bg-white border-x-[#000000] p-4`}>YEAR</span>
                    <div className="relative">
                        <button
                            key="years"

                            className={`${geistMono.className} flex justify-center items-center text-sm bg-white tracking-wide font-medium text-black p-4`}
