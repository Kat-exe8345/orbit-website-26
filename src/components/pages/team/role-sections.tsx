...
export default function RoleSections({ sections, activeTab }: { sections: Section[], activeTab: number | "all_teams" }) {
    ...
    if (!sections || sections.length === 0) {
        return (
            <div className="flex flex-col shrink-0 h-full items-center w-full mt-12.5 py-12.5 gap-4">
                <div className="flex justify-center items-center">
                    <HeartCrack className="w-16 h-16 text-[#ff4545]" />
                </div>
                <div className={`${inter.className} max-lg:text-lg text-xl pt-4 w-full flex flex-col justify-center items-center font-normal text-center text-white`}>
                    "{selectedLine}"
                </div>
            </div>
        );
